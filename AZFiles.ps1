...
Param(
    ...
    [Parameter(Mandatory = $false)]
    [string]$LogPath = "C:\temp\StorageAccount.log", 
...
$ModulePath = "C:\temp\" #Output path for modules
...
function StartIteration {
	$FolderName = "c:\temp\"
	$FolderName2 = "c:\temp\AzFilesHybrid\"
	...
	if (Test-Path $FolderName2) {
       Write-Host "Folder2 Exists"
	}
	else{
    #PowerShell Create directory if not exists
    New-Item $FolderName2 -ItemType Directory
	Write-Host "Folder2 Created successfully"
	}
...
function ValidateStorageAccountNameCharacterCount {
    if ($StorageAccountName.length -gt 25 ) {
...
if ($JSON.IsPresent) {
    ...
    $AppId = $EnvironmentDetails.AppId #AppId
    $AppSecret = $EnvironmentDetails.AppSecret #AppSecret
...
Write-Log -Message "Tenant ID is set to: $($TenantID)" -Level Info
...
if ($null -ne $PowerShellGet) {
    ...
    Write-Log -Message "Importing Module: PowerShellGet" -Level Info
        ...
        try {
            ...
            Install-Module -Name "PowerShellGet" -AllowClobber -force -SkipPublisherCheck -ErrorAction Stop -Confirm:$false
            Import-Module -Name "PowerShellGet" -Force
        }
        catch {
            Write-Log -Message $_ -Level Warn
...
if ($null -ne $AzureADModule) {
    Write-Log -Message "AzureAd version $($AzureADModule.Version) is installed" -Level Info
    Import-Module -Name "AzureAd" -Force
}
else {
    try {
        Write-Log -Message "AzureAd is not installed. Attempting Install" -Level Info
        Install-Module -Name "AzureAd" -AllowClobber -ErrorAction Stop -Confirm:$false -force
        Import-Module -Name "AzureAd" -Force
		Write-Log -Message "AzureAd Module installed" -Level Info
    }
    catch {
        Write-Log -Message "Failed to Import Module AzureAd. Exiting" -Level Warn
        Write-Log -Message $_ -Level Warn
        StopIteration
        Exit 1
    }    
...
if ($null -ne $AZStorage) {
    ...
    try {
        ...
        Install-Module -Name "AZ.Storage"-AllowClobber -ErrorAction Stop -Confirm:$false -force
        Import-Module -Name "AZ.Storage" -Force
    }
    catch {
        Write-Log -Message "Failed to Import Module AZ.Storage. Exiting" -Level Warn
...
$AZNetWork = (Get-Module -Name "AZ.Network")
if ($null -ne $AZNetWork) {
    Write-Log -Message "AZ.Network version $($AZNetWork.Version) is installed" -Level Info
    Import-Module -Name "AZ.Network" -Force
}
else {
    try {
        Write-Log -Message "AZ.Network is not installed. Attempting Install" -Level Info
        Install-Module -Name "AZ.Network"-AllowClobber -ErrorAction Stop -Confirm:$false -force
        Import-Module -Name "AZ.Network" -Force
    }
    catch {
        Write-Log -Message "Failed to Import Module AZ.Network. Exiting" -Level Warn
        Write-Log -Message $_ -Level Warn
        StopIteration
        Exit 1
    }    
...
try {
	...
	Connect-AzAccount -ServicePrincipal -Credential $pscredential -Tenant $tenantId -Subscription $SubscriptionId -ErrorAction Stop
